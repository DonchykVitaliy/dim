...
execute if score @s pir.shell.bb matches 1..158 run tp @s @e[type=marker,limit=1,distance=..2,tag=pir.coordinate_shell]

execute if score @s pir.shell.bb matches 1 run summon marker ~ ~ ~ {Tags:["pir.coordinate_shell"]}
...
execute if score @s pir.shell.bb matches 159 run kill @e[type=marker,tag=pir.coordinate_shell,distance=..1]
